...
export const getUsers = query({
	...
	handler: async (ctx, args) => {
		...
		// console.log('User identity:', identity); // Log giá trị của identity
		if (!identity) {
...
export const getMe = query({
	...
	handler: async (ctx, args) => {
		const identity = await ctx.auth.getUserIdentity();
		if (!identity) {
			throw new ConvexError("Unauthorized: No identity found");
		}

		// console.log("Checking for user with tokenIdentifier:", identity.tokenIdentifier);
		...
		if (!user) {
			throw new ConvexError("User not found with provided tokenIdentifier");
		}
